...
@section('main-content')
    <!-- Page Heading -->
    <h1 class="h3 mb-4 text-gray-800">{{ __('Edit Pesan Pernikahan') }}</h1>

    <div class="row">
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header d-flex justify-content-between">
                    <h5>Edit Data Pesan Pernikahan</h5>
                    ...
                    <form action="{{ route('messages.update', $message->id) }}" method="POST">
                            ...
                            <select class="form-control" id="wedding_id" name="wedding_id" required>
                                @foreach ($weddings as $wedding)
                                    <option value="{{ $wedding->id }}"
                                        {{ $message->wedding_id == $wedding->id ? 'selected' : '' }}>{{ $wedding->title }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="guest_name" name="guest_name"
                                value="{{ $message->guest_name }}" required>
                        </div>
                        <div class="form-group">
                            <label for="attendance">Kehadiran</label>
                            <select class="form-control" id="attendance" name="attendance" required>
                                <option value="Hadir" {{ $message->attendance == 'Hadir' ? 'selected' : '' }}>Hadir
                                </option>
                                <option value="Tidak Hadir" {{ $message->attendance == 'Tidak Hadir' ? 'selected' : '' }}>
                                    Tidak Hadir</option>
                            </select>
                        </div>
                        <div class="form-group">
